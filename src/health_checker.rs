...
use crate::health_checker::redis_health_checker::Http;
...
fn load_port_from(vars: &HashMap<String, String>) -> Result<u16, InvalidConfiguration> {
    let env_var = vars.get(env!("PORT")).or(vars.get("PORT"));
...
) -> Result<Configuration, InvalidConfiguration> {
...
trait PingCommand {
    fn run(&self, connection: &mut Connection) -> bool;
}

struct TcpPingCommand;

impl PingCommand for TcpPingCommand {
    fn run(&self, connection: &mut Connection) -> bool {
        connection.check_connection()
    }
}

struct RedisHealthChecker<'a> {
    ping_command: &'a (dyn PingCommand + 'a),
}

impl<'a> HealthChecker for RedisHealthChecker<'a> {
    async fn check(&self, configuration: &Configuration) -> Result<State, NetworkError> {
        let mut url = Url::parse("redis://localhost").unwrap();
        url.set_port(Some(configuration.port)).unwrap();
        let client = redis::Client::open(url).unwrap();

        let mut connection = client.get_connection().map_err(|e| NetworkError {
            message: format!("network error: {}", e),
        })?;

        let command_result = self.ping_command.run(&mut connection);
...
async fn get_health(configuration: &Configuration) -> Result<State, NetworkError> {
    Http {}.check(&configuration).await
}

pub async fn run_health_check() -> Result<State, HeathcheckFailure> {
    let vars: HashMap<String, String> = env::vars().collect();

    let configuration = load_configuration_from(vars).map_err(|err| HeathcheckFailure {
